...
from modeling_albertX import AlbertXPretraining, set_visual_config
...
def convert_example_to_features(example: InputExample, max_seq_length, tokenizer)->InputFeatures:
    ...
    tokens = tokenizer.tokenize(example.sent)
...
LOSSES_NAME = ('Mask_LM', 'Matched', 'Obj', 'Feat')
...
class LXMERT:
    def __init__(self, max_seq_length):
        ...
        self.model = AlbertXPretraining.from_pretrained(
            args.tr,
    ...
    def save(self, name):
        torch.save(self.model.state_dict(),
                   os.path.join(args.output, "%s_AX.pth" % name))
