...
def clean_data(data_path="./data"):
    ...
    dev_seen = pd.read_json("../input/fbmdatafinal/fbmdatafinal/data/dev_seen.jsonl", lines=True, orient="records")
    # We validate with dev_seen throughout all experiments, so we only take the new data from dev_unseen add it to train and then discard it
    dev_unseen = pd.read_json("../input/fbmdatafinal/fbmdatafinal/data/dev_unseen.jsonl", lines=True, orient="records")
    dev_unseen = dev_unseen[~dev_unseen['id'].isin(dev_seen.id.values)].copy()

    df_dict = {'train': train, 'dev': dev_seen, 'dev_unseen': dev_unseen}
    ...
    trainclean.to_json(path_or_buf='/kaggle/working/traincleanex.jsonl', orient='records', lines=True)
    traincleandev.to_json(path_or_buf='/kaggle/working/traincleandevex.jsonl', orient='records', lines=True)
    full.to_json(path_or_buf='/kaggle/working/fullfinex.jsonl', orient='records', lines=True)


def combine_data(data_path="./data"):
    """
    Combines the train, dev & dev_unseen data to form a final train, traindev & dev set. 
    """
    ...
    df_dict = {'train': train, 'dev_unseen': dev_unseen ,'dev': missing_data}
    train_dist = pd.concat([df.assign(identity=key) for key,df in df_dict.items()])
    train_dist = train_dist.drop('ident', 1)
...
def create_subdata(data_path="./data"):
    ...
    traincleanex = pd.read_json(data_path + '/traincleanex.jsonl', lines=True)
    devseen = pd.read_json(data_path + '/devseen.jsonl', lines=True)
    test = pd.read_json(data_path + '/test.jsonl', lines=True)
    test_unseen = pd.read_json(data_path + '/test_unseen.jsonl', lines=True)
