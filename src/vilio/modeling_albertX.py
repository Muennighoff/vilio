...
class LXRTEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Make sure albert layers equal specified l layers
        #config.num_hidden_layers = self.num_l_layers - Just using all Albert layers seems to perform best

        config.num_hidden_layers = 9

        # Number of Albert Layer groups (All Albert models have just 1 Layergroup - The Layer group itself then contains 12-24 layers)
        self.embedding_hidden_mapping_in = nn.Linear(config.embedding_size, config.hidden_size)
        self.albert_layer_groups = nn.ModuleList([AlbertLayerGroup(config) for _ in range(config.num_hidden_groups)])

        ### For xxl model adjust intermediate size here for remaining layers
        config.intermediate_size = config.hidden_size ## 3072 BBU; axxl hidden = 4096
    ...
    def forward(self, lang_feats, lang_attention_mask,
            visn_feats, visn_attention_mask=None, head_mask=None):
        # Run visual embedding layer
        # Note: Word embedding layer was executed outside this module.
        #       Keep this design to allow loading BERT weights.
        visn_feats = self.visn_fc(visn_feats)

        # Run ALL (N(L) is ignored) Lang Encoders
        hidden_states = self.embedding_hidden_mapping_in(lang_feats)
...
class AlbertX(BertPreTrainedModel):
    """LXRT Model."""
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                visual_feats=None, visual_attention_mask=None, head_mask=None, position_ids=None, inputs_embeds=None):
        """
        head_mask: Masks attention heads for pre-training.
                   We leave it as none & skip the functions relating to it, as we currently do not implement masked pre-training.
        """
        ...
        pooled_output = self.pooler_activation(self.pooler(lang_feats[:, 0]))
...
class BertLMPredictionHead(nn.Module):
    def __init__(self, config, bert_model_embedding_weights):
        ...
        # Albert: hidden_size != embedding_size, hence we adopt a mapping in
        self.embedding_hidden_mapping_in = nn.Linear(config.hidden_size, config.embedding_size)
...
class AlbertMLMHead(nn.Module):
    """
    AlbertMLMHead from transformers + seg_relationship from LXMERT
    """
    def __init__(self, config):
        super().__init__()
...
class AlbertXPretraining(BertPreTrainedModel):
    ...
    def __init__(self,
        ...
        # LXRT backbone
        self.albert = AlbertX(config, llayers=llayers, xlayers=xlayers, rlayers=rlayers)

        # Pre-training heads
        self.cls = BertPreTrainingHeads(config, self.albert.embeddings.word_embeddings.weight)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
        ...
        (lang_output, visn_output), pooled_output = self.albert(
