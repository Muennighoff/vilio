...
from src.vilio.modeling_albertV import AlbertVPretraining
    ...
    for i, token in enumerate(tokens):
        ...
        if prob < ratio:
            ...
            try:
                ...
                output_label.append(tokenizer.get_vocab()["<unk>"]) # <unk> in ALBERT
        else:
...
LOSSES_NAME = ('Mask_LM', 'Matched', 'Feat') #Removed , 'Matched', 'Obj', 'Feat' 'Attr', 'QA'
...
class LXMERT:
    def __init__(self, max_seq_length):
        ...
        print("RESETTING TOKTYPEIDS!")
        self.model.albert.embeddings.token_type_embeddings.weight.data.normal_(mean=0.0, std=self.model.albert.config.initializer_range)
    ...
    def train_batch(self, optim, scheduler, batch, ups, accumulation_steps=args.acc):
    ...
    def train(self, train_tuple: DataTuple, eval_tuple: DataTuple, accumulation_steps=args.acc):
        train_ld = train_tuple.loader

        # Optimizer
        batch_per_epoch = len(train_ld)
        t_total = int(batch_per_epoch * args.epochs  // accumulation_steps)
        warmup_ratio = 0.05
    ...
    def save(self, name):
        torch.save(self.model.state_dict(),
                   os.path.join(args.output, "%s_AV.pth" % name))
